...
**Size filter on the catalogue (same evening, owner request).** `/collections/all` and `/collections/<handle>` got a "Méret" chip row under the filter bar (`app/components/SizeFilter.tsx`, `app/lib/sizes.ts`): one chip per size that at least one product is in stock in, `?size=M` keeps only the products available in that size, the active chip toggles off. A one-off sized "M-L" counts as M and L, so the chips stay the standard XS–XXL run. The data is the card query's variants (`availableForSale` + `selectedOptions`), added outside the shared card fragment so the other routes are untouched; `?size=` joined `?sort=` in robots.txt. Verified locally against Storefront counts (M: 30 of 42, tees in L: 17 of 25).

**Discount expiry (same evening).** The owner expired "Nyitási akció" and created "Ez egy akcoóó" (80% off Kabátok és blézerek above 50 000 Ft, until 2026-09-13). The Admin API list is memoised per worker; the memo went from 5 minutes to 1, and campaigns whose end date has passed are dropped on every request, so an expiry shows within a minute at most. Several campaigns are supported: `/akcio` lists all of them (buy-X-get-Y first, then percentage/amount, then shipping), the announcement bar and hero show the first one, cards and product pages show the first campaign that applies to that product, and the cart nudge only reacts to buy-X-get-Y. Non-combinable discounts: Shopify applies one of them in the cart.
